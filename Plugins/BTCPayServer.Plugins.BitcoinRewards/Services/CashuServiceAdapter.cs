...
public class CashuServiceAdapter : ICashuService
{
    ...
    private async Task<string?> GetMintUrlForStore(string storeId)
    {
        try
        {
            ...
            if (_paymentMethodHandlers != null)
            {
                _logger.LogInformation("Attempting to get mint URL from payment method config for store {StoreId}", storeId);
                ...
                if (cashuPluginType != null)
                {
                    ...
                    if (cashuPmidField != null)
                    {
                        ...
                        if (cashuPmid != null)
                        {
                            _logger.LogInformation("Found CashuPmid: {CashuPmid}", cashuPmid);
                            
                            // Try GetPaymentMethodConfig<T> generic method (extension method, so check extension method class)
                            // Search in all loaded assemblies for StoreDataExtensions
                            Type? storeDataExtensionsType = null;
                            foreach (var assembly in AppDomain.CurrentDomain.GetAssemblies())
                            {
                                try
                                {
                                    storeDataExtensionsType = assembly.GetTypes()
                                        .FirstOrDefault(t => t.Name == "StoreDataExtensions" && 
                                                           t.Namespace == "BTCPayServer.Data");
                                    if (storeDataExtensionsType != null)
                                    {
                                        _logger.LogInformation("Found StoreDataExtensions in assembly: {Assembly}", assembly.GetName().Name);
                                        break;
                                    }
                                }
                                catch
                            ...
                            MethodInfo? getPaymentMethodConfigGenericMethod = null;
                            if (storeDataExtensionsType != null)
                            {
                                var allMethods = storeDataExtensionsType
                                    .GetMethods(BindingFlags.Public | BindingFlags.Static)
                                    .Where(m => m.Name == "GetPaymentMethodConfig" && m.IsGenericMethod)
                                    .ToList();
                                
                                _logger.LogInformation("Found {Count} GetPaymentMethodConfig methods in StoreDataExtensions", allMethods.Count);
                                
                                // Try with 4 parameters first (with onlyEnabled)
                                getPaymentMethodConfigGenericMethod = allMethods
                                    .FirstOrDefault(m => m.GetParameters().Length == 4);
                                
                                if (getPaymentMethodConfigGenericMethod == null)
                                {
                                    // Try with 3 parameters (without onlyEnabled)
                                    getPaymentMethodConfigGenericMethod = allMethods
                            ...
                            {
                                _logger.LogInformation("StoreDataExtensions type not found in any loaded assembly");
                            }
                            
                            if (getPaymentMethodConfigGenericMethod != null)
                            {
                                _logger.LogInformation("Found GetPaymentMethodConfig<T> generic method");
                                var cashuConfigType = _cashuAssembly?.GetType("BTCPayServer.Plugins.Cashu.PaymentHandlers.CashuPaymentMethodConfig");
                                if (cashuConfigType != null)
                                {
                                    _logger.LogInformation("Found CashuPaymentMethodConfig type: {Type}", cashuConfigType.FullName);
                                    var getPaymentMethodConfigMethod = getPaymentMethodConfigGenericMethod.MakeGenericMethod(cashuConfigType);
                                    // Extension method: first parameter is 'this StoreData', then PaymentMethodId, handlers, optional onlyEnabled
                                    var parameters = getPaymentMethodConfigMethod.GetParameters();
                                    object[] invokeParams = parameters.Length == 3
                                        ? new object[] { store, cashuPmid, _paymentMethodHandlers }
                                        : new object[] { store, cashuPmid, _paymentMethodHandlers, false };
                                    var config = getPaymentMethodConfigMethod.Invoke(null, invokeParams);
                                    if (config != null)
                                    {
                                        _logger.LogInformation("Found CashuPaymentMethodConfig for store {StoreId}, config type: {ConfigType}", storeId, config.GetType().FullName);
                                        ...
                                        if (trustedMintsProperty != null)
                                        {
                                            ...
                                            {
                                                _logger.LogInformation("TrustedMintsUrls is null or empty for store {StoreId} (count: {Count})", storeId, trustedMints?.Count ?? 0);
                                            }
                                        }
                                        else
                                        {
                                            _logger.LogInformation("TrustedMintsUrls property not found on config type {ConfigType}. Available properties: {Properties}", 
                                                config.GetType().FullName, 
                                                string.Join(", ", config.GetType().GetProperties().Select(p => p.Name)));
                                        }
                                    }
                                    else
                                    {
                                        _logger.LogInformation("GetPaymentMethodConfig returned null for store {StoreId}", storeId);
                                    }
                                }
                                else
                                {
                                    _logger.LogInformation("CashuPaymentMethodConfig type not found in Cashu assembly");
                                }
                            }
                            else
                            {
                                _logger.LogInformation("GetPaymentMethodConfig<T> generic method not found in StoreDataExtensions");
                            }
                            
                            // Fallback: Try non-generic method
                            var getPaymentMethodConfigMethodFallback = typeof(StoreData).GetMethod("GetPaymentMethodConfig", 
                                new[] { cashuPmid.GetType(), _paymentMethodHandlers.GetType() });
                            if (getPaymentMethodConfigMethodFallback != null)
                            {
                                _logger.LogInformation("Trying fallback GetPaymentMethodConfig (non-generic) for store {StoreId}", storeId);
                                var config = getPaymentMethodConfigMethodFallback.Invoke(store, new[] { cashuPmid, _paymentMethodHandlers });
                                if (config != null)
                                {
                                    _logger.LogInformation("Fallback GetPaymentMethodConfig returned config type: {ConfigType}", config.GetType().FullName);
                                    // Get TrustedMintsUrls property
                                    var trustedMintsProperty = config.GetType().GetProperty("TrustedMintsUrls");
                                    if (trustedMintsProperty != null)
                                    {
                                        ...
                                        {
                                            var mintUrl = trustedMints.First();
                                            _logger.LogInformation("Found mint URL from payment method config (fallback) for store {StoreId}: {MintUrl}", storeId, mintUrl);
                                            return mintUrl;
                                        }
                                        else
                                        {
                                            _logger.LogInformation("Fallback: TrustedMintsUrls is null or empty for store {StoreId} (count: {Count})", storeId, trustedMints?.Count ?? 0);
                                        }
                                    }
                                    else
                                    {
                                        _logger.LogInformation("Fallback: TrustedMintsUrls property not found on config type {ConfigType}", config.GetType().FullName);
                                    }
                                }
                                else
                                {
                                    _logger.LogInformation("Fallback GetPaymentMethodConfig returned null for store {StoreId}", storeId);
                                }
                            }
                            else
                            {
                                _logger.LogInformation("Fallback GetPaymentMethodConfig method not found for store {StoreId}", storeId);
                            }
                        }
                        else
                        {
                            _logger.LogInformation("CashuPmid field value is null");
                        }
                    }
                    else
                    {
                        _logger.LogInformation("CashuPmid field not found on CashuPlugin type");
                    }
                }
                else
                {
                    _logger.LogInformation("CashuPlugin type not found in assembly");
                }
            }
            else
            {
                _logger.LogInformation("PaymentMethodHandlerDictionary is null");
            }

            // Method 2: Try to get from database (mints that have proofs for this store)
            _logger.LogInformation("Attempting to get mint URL from database for store {StoreId}", storeId);
            if (_cashuDbContextFactory != null)
            {
                try
                {
                    // Get CreateContext method - try with no parameters first, then with optional Action parameter
                    ...
                    {
                        // Try to find method with optional Action parameter using reflection
                        var allMethods = _cashuDbContextFactory.GetType().GetMethods(BindingFlags.Public | BindingFlags.Instance)
                            .Where(m => m.Name == "CreateContext" && m.GetParameters().Length == 1)
                            .ToList();
                        
                        if (allMethods.Count > 0)
                        {
                            // Use the first one and pass null for the optional parameter
                            createContextMethod = allMethods[0];
                        }
                    }
                    
                    if (createContextMethod != null)
                    {
                        // Call with appropriate parameters (null for optional parameter if needed)
                        object?[] parameters = createContextMethod.GetParameters().Length == 0 
                            ? Array.Empty<object>() 
                            : new object?[] { null };
                        _logger.LogInformation("Calling CreateContext on CashuDbContextFactory");
                        var dbContext = createContextMethod.Invoke(_cashuDbContextFactory, parameters);
                        if (dbContext != null)
                        {
                            ...
                            try
                            {
                                ...
                                if (mintsProperty != null)
                                {
                                    ...
                                    if (mintsDbSet != null)
                                    {
                                        _logger.LogInformation("Found Mints DbSet, querying for mint URLs");
                                        ...
                                        {
                                            // Get all mints and return the first one with a URL
                                            ...
                                            object?[] mintsToListParams = toListAsyncMethod.GetParameters().Length == 1
                                                ? new object[] { mintsDbSet }
                                                : new object[] { mintsDbSet, CancellationToken.None };
                                            
                                            var mintsTask = mintsToListAsync.Invoke(null, mintsToListParams) as Task;
                                            ...
                                            if (mintsTask != null)
                                            {
                                                ...
                                                if (mintsList != null)
                                                {
                                                    ...
                                                    foreach (var mint in mintsList)
                                                    {
                                                        ...
                                                        {
                                                            _logger.LogInformation("Found mint URL from database for store {StoreId}: {MintUrl} (from {Count} mints)", storeId, mintUrl, mintCount);
                                                            return mintUrl;
                                                        }
                                                    }
                                                    _logger.LogInformation("Found {Count} mints in database but none have URLs", mintCount);
                                                }
                                                else
                                                {
                                                    _logger.LogInformation("Mints query returned null");
                                                }
                                            }
                                            else
                                            {
                                                _logger.LogInformation("ToListAsync invocation returned null");
                                            }
                                        }
                                        else
                                        {
                                            _logger.LogInformation("ToListAsync method not found");
                                        }
                                    }
                                    else
                                    {
                                        _logger.LogInformation("Mints DbSet is null");
                                    }
                                }
                                else
                                {
                                    _logger.LogInformation("Mints property not found on DbContext type: {Type}", dbContext.GetType().FullName);
                                }
                            }
                            catch (Exception ex)
                            {
                                _logger.LogWarning(ex, "Error querying database for mint URL: {Message}", ex.Message);
                            }
                            finally
                            {
                        ...
                        {
                            _logger.LogInformation("CreateContext returned null");
                        }
                    }
                }
                catch (Exception ex)
                {
                    _logger.LogWarning(ex, "Error getting mint URL from database for store {StoreId}", storeId);
                }
            }
    ...
    private async Task<long> GetEcashBalanceAsync(string storeId, string mintUrl)
            ...
            // Create database context - handle optional parameter
            // CreateContext has signature: CreateContext(Action<NpgsqlDbContextOptionsBuilder> npgsqlOptionsAction = null)
            var factoryType = _cashuDbContextFactory.GetType();
            var allCreateContextMethods = factoryType.GetMethods(BindingFlags.Public | BindingFlags.Instance)
                .Where(m => m.Name == "CreateContext")
                .ToList();
            
            MethodInfo? createContextMethod = null;
            object?[]? parameters = null;
            
            // Try to find method with 1 optional parameter (Action<NpgsqlDbContextOptionsBuilder>)
            createContextMethod = allCreateContextMethods.FirstOrDefault(m => 
            {
                var paramInfos = m.GetParameters();
                return paramInfos.Length == 1 && 
                       paramInfos[0].IsOptional &&
                       paramInfos[0].ParameterType.Name.Contains("Action");
            });
            
            if (createContextMethod != null)
            {
                // Call with null for optional parameter
                parameters = new object?[] { null };
            }
            else
            {
                // Try method with 0 parameters
                createContextMethod = allCreateContextMethods.FirstOrDefault(m => m.GetParameters().Length == 0);
                if (createContextMethod != null)
                {
            ...
            if (createContextMethod == null)
            {
                _logger.LogWarning("CreateContext method not found on CashuDbContextFactory. Found {Count} methods with name CreateContext", allCreateContextMethods.Count);
                return 0;
            }
            ...
            try
            {
                ...
                // GetKeysets might be async and require CancellationToken
                var allGetKeysetsMethods = cashuHttpClient.GetType().GetMethods(BindingFlags.Public | BindingFlags.Instance)
                    .Where(m => m.Name == "GetKeysets")
                    .ToList();
                
                if (allGetKeysetsMethods.Count == 0)
                {
                    _logger.LogWarning("GetKeysets method not found on CashuHttpClient");
                    return 0;
                }
                
                // Prefer method with 0 parameters, fallback to method with CancellationToken
                var getKeysetsMethod = allGetKeysetsMethods.FirstOrDefault(m => m.GetParameters().Length == 0)
                    ?? allGetKeysetsMethods.FirstOrDefault(m => 
                        m.GetParameters().Length == 1 && 
                        m.GetParameters()[0].ParameterType == typeof(CancellationToken));
                
                if (getKeysetsMethod == null)
                {
                    _logger.LogWarning("No suitable GetKeysets method found. Found {Count} overloads", allGetKeysetsMethods.Count);
                    return 0;
                }

                // Invoke with appropriate parameters
                object?[] keysetsParams = getKeysetsMethod.GetParameters().Length == 0 
                    ? Array.Empty<object>() 
                    : new object[] { CancellationToken.None };
                var keysetsTask = getKeysetsMethod.Invoke(cashuHttpClient, keysetsParams) as Task;
                if (keysetsTask == null)
                {
                ...
                // Get keyset IDs - store as objects for proper comparison
                var keysetIds = new List<object>();
                ...
                var allToListAsyncMethods = typeof(EntityFrameworkQueryableExtensions)
                    .GetMethods(BindingFlags.Public | BindingFlags.Static)
                    .Where(m => m.Name == "ToListAsync")
                    .ToList();
                
                // Prefer method with 1 parameter (IQueryable), fallback to method with 2 parameters (IQueryable + CancellationToken)
                var toListAsyncMethod = allToListAsyncMethods.FirstOrDefault(m => 
                    m.GetParameters().Length == 1 && 
                    m.GetParameters()[0].ParameterType.Name.Contains("IQueryable"))
                    ?? allToListAsyncMethods.FirstOrDefault(m => 
                ...
                if (toListAsyncMethod == null)
                {
                    _logger.LogWarning("ToListAsync method not found. Found {Count} overloads", allToListAsyncMethods.Count);
                    return 0;
                }
                ...
                // Invoke with appropriate parameters
                object?[] toListParams = toListAsyncMethod.GetParameters().Length == 1
                    ? new object[] { proofsDbSet }
                    : new object[] { proofsDbSet, CancellationToken.None };
                
                var proofsTask = toListAsyncGeneric.Invoke(null, toListParams) as Task;
                if (proofsTask == null)
                {
                ...
                foreach (var proof in proofsList)
                {
                    ...
                    {
                        var proofId = idProperty.GetValue(proof);
                        
                        // Check if proof's keyset ID matches any of the active keyset IDs
                        // Compare using Equals method or direct object comparison
                        bool matchesKeyset = false;
                        foreach (var keysetId in keysetIds)
                        {
                            if (proofId != null && keysetId != null)
                            {
                                // Try Equals method first
                                var equalsMethod = proofId.GetType().GetMethod("Equals", new[] { keysetId.GetType() });
                                if (equalsMethod != null)
                                {
                                    var equalsResult = equalsMethod.Invoke(proofId, new[] { keysetId });
                                    if (equalsResult is bool result && result)
                                    {
                                        matchesKeyset = true;
                                        break;
                                    }
                                ...
                                else if (proofId.ToString() == keysetId.ToString())
                                {
                                    matchesKeyset = true;
                                    break;
                                }
                        ...
                        if (matchesKeyset)
                        {
                            // Check if proof is in FailedTransactions
                            var failedTransactionCheck = true; // Assume not failed for now
                            
                            if (failedTransactionCheck)
                            {
                ...
                _logger.LogDebug("Found {ProofCount} proofs matching keysets for store {StoreId}", proofCount, storeId);
    ...
    private async Task<long> GetLightningBalanceAsync(string storeId)
    {
        try
        {
            ...
            // Get GetNetwork method - handle ambiguous match by finding exact signature
            var allGetNetworkMethods = networkProviderType.GetMethods(BindingFlags.Public | BindingFlags.Instance)
                .Where(m => m.Name == "GetNetwork")
                .ToList();
            
            var getNetworkMethod = allGetNetworkMethods.FirstOrDefault(m => 
                m.GetParameters().Length == 1 && 
                m.GetParameters()[0].ParameterType == typeof(string) &&
                m.ReturnType.Name.Contains("BTCPayNetwork"));
            
            if (getNetworkMethod == null)
            {
                _logger.LogWarning("GetNetwork(string) method not found. Found {Count} overloads", allGetNetworkMethods.Count);
                return 0;
            }
    ...
    private async Task<List<object>?> GetStoredProofsAsync(string storeId, string mintUrl, ulong maxAmount)
    {
        try
        {
            ...
            // Get proofs from database - handle optional parameter
            var factoryType = _cashuDbContextFactory.GetType();
            var allCreateContextMethods = factoryType.GetMethods(BindingFlags.Public | BindingFlags.Instance)
                .Where(m => m.Name == "CreateContext")
                .ToList();
            ...
            // Try to find method with 1 optional parameter (Action)
            createContextMethod = allCreateContextMethods.FirstOrDefault(m => 
                m.GetParameters().Length == 1 && 
                m.GetParameters()[0].IsOptional &&
                m.GetParameters()[0].ParameterType.Name.Contains("Action"));
            ...
            try
            {
                ...
                if (toListAsyncMethod == null)
                {
                    _logger.LogWarning("ToListAsync method not found in GetStoredProofsAsync. Found {Count} overloads", allToListAsyncMethods.Count);
                    return null;
                }
                ...
                // Invoke with appropriate parameters
                object?[] proofsToListParams = toListAsyncMethod.GetParameters().Length == 1
                    ? new object[] { proofsDbSet }
                    : new object[] { proofsDbSet, CancellationToken.None };
                
                var proofsTask = toListAsyncGeneric.Invoke(null, proofsToListParams) as Task;
                if (proofsTask == null)
                {
                    return null;
                }
                ...
                // Get FailedTransactions to exclude them
                var failedTransactionsProperty = dbContext.GetType().GetProperty("FailedTransactions");
                var failedTransactionsDbSet = failedTransactionsProperty?.GetValue(dbContext);
                var failedTransactionsList = new List<object>();
                
                if (failedTransactionsDbSet != null)
                {
                    var allFailedTransactionsToListMethods = typeof(EntityFrameworkQueryableExtensions)
                        .GetMethods(BindingFlags.Public | BindingFlags.Static)
                        .Where(m => m.Name == "ToListAsync")
                        .ToList();
                    
                    var failedTransactionsToListMethod = allFailedTransactionsToListMethods.FirstOrDefault(m => 
                        m.GetParameters().Length == 1 && 
                        m.GetParameters()[0].ParameterType.Name.Contains("IQueryable"))
                        ?? allFailedTransactionsToListMethods.FirstOrDefault(m => 
                            m.GetParameters().Length == 2 && 
                            m.GetParameters()[0].ParameterType.Name.Contains("IQueryable") &&
                            m.GetParameters()[1].ParameterType == typeof(CancellationToken));
                    
                    if (failedTransactionsToListMethod != null)
                    {
                        var failedTransactionsToListGeneric = failedTransactionsToListMethod.MakeGenericMethod(
                            failedTransactionsDbSet.GetType().GetGenericArguments()[0]);
                        
                        // Invoke with appropriate parameters
                        object?[] failedTransactionsToListParams = failedTransactionsToListMethod.GetParameters().Length == 1
                            ? new object[] { failedTransactionsDbSet }
                            : new object[] { failedTransactionsDbSet, CancellationToken.None };
                        
                        var failedTransactionsTask = failedTransactionsToListGeneric.Invoke(null, failedTransactionsToListParams) as Task;
                        if (failedTransactionsTask != null)
                        {
                            await failedTransactionsTask;
                            var failedTransactionsResult = failedTransactionsTask.GetType().GetProperty("Result")?.GetValue(failedTransactionsTask) as System.Collections.IEnumerable;
                            if (failedTransactionsResult != null)
                            {
                                foreach (var ft in failedTransactionsResult)
                                {
                                    failedTransactionsList.Add(ft);
                                }
                            }
                        }
                    }
                ...
                // Get UsedProofs from FailedTransactions
                var usedProofsSet = new HashSet<object>();
                foreach (var failedTransaction in failedTransactionsList)
                {
                    var usedProofsProperty = failedTransaction.GetType().GetProperty("UsedProofs");
                    var usedProofs = usedProofsProperty?.GetValue(failedTransaction) as System.Collections.IEnumerable;
                    if (usedProofs != null)
                    {
                        foreach (var usedProof in usedProofs)
                        {
                            usedProofsSet.Add(usedProof);
                        }
                    }
                }

                // Filter proofs by store, keyset, exclude failed transactions, and amount
                ...
                foreach (var proof in proofsList)
                {
                        ...
                        idProperty?.GetValue(proof) != null)
                    {
                        ...
                        if (matchesKeyset)
                        {
                            // Exclude proofs that are in FailedTransactions
                            if (!usedProofsSet.Contains(proof))
                            {
                                var amount = amountProperty?.GetValue(proof);
                                if (amount != null && ulong.TryParse(amount.ToString(), out var amountValue))
                                {
    ...
    private async Task<string?> SwapProofsAsync(ulong amount, string mintUrl, string storeId)
    {
        try
        {
            ...
            _logger.LogInformation("Swapping proofs for {Amount} sat on mint {MintUrl} for store {StoreId}", 
                amount, mintUrl, storeId);

            // Get stored proofs
            var proofs = await GetStoredProofsAsync(storeId, mintUrl, amount);
            if (proofs == null || proofs.Count == 0)
            {
                _logger.LogWarning("No proofs available for swapping {Amount} sat on mint {MintUrl} for store {StoreId}", 
                    amount, mintUrl, storeId);
                return null;
            }

            _logger.LogDebug("Retrieved {ProofCount} proofs for swapping", proofs.Count);
            ...
            var token = await CreateTokenFromProofs(resultProofList, mintUrl, "sat");
            if (token != null)
            {
                _logger.LogInformation("Successfully created token from {ProofCount} swapped proofs for {Amount} sat", 
                    resultProofList.Count, amount);
            }
            else
            {
                _logger.LogWarning("Failed to create token from swapped proofs");
            }
            return token;
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "Error swapping proofs for {Amount} sat on mint {MintUrl} for store {StoreId}", 
                amount, mintUrl, storeId);
            return null;
        }
    }
    ...
    private async Task<List<object>?> MintFromLightningAsync(long amountSatoshis, string storeId, string mintUrl)
    {
        try
        {
            ...
            var networkProvider = _serviceProvider.GetService(networkProviderType);
            if (networkProviderType == null || networkProvider == null)
            {
                return null;
            }
            
            // Get GetNetwork method - handle ambiguous match
            var allGetNetworkMethods = networkProviderType.GetMethods(BindingFlags.Public | BindingFlags.Instance)
                .Where(m => m.Name == "GetNetwork")
                .ToList();
    ...
    private Task<string?> CreateTokenFromProofs(List<object> proofs, string mintUrl, string unit)
    {
        try
        {
            ...
            // Create Token object - CashuToken.Token is a nested class
            // First get all nested types from CashuToken
            var tokenItemType = cashuTokenType.GetNestedTypes(BindingFlags.Public | BindingFlags.NonPublic)
            ...
            if (tokenItem == null)
            {
                _logger.LogWarning("Failed to create CashuToken.Token instance");
                return Task.FromResult<string?>(null);
            }
            ...
            // Encode token - use instance method Encode() on CashuToken
            var encodeMethod = cashuTokenType.GetMethod("Encode", Type.EmptyTypes);
            if (encodeMethod == null)
            {
                _logger.LogWarning("CashuToken.Encode() method not found");
                return Task.FromResult<string?>(null);
            }
            ...
            {
                _logger.LogWarning("Token encoding returned null");
                return Task.FromResult<string?>(null);
            }

            var serializedToken = encodedToken.ToString();
            _logger.LogInformation("Successfully created and encoded Cashu token with {ProofCount} proofs for {MintUrl}", 
                proofs.Count, mintUrl);
    ...
    public async Task<string?> MintTokenAsync(long amountSatoshis, string storeId)
    {
        ...
        try
        {
            ...
            // 2. Check ecash balance first - use existing proofs if available
            ...
            if (ecashBalance >= amountSatoshis)
            {
                ...
                _logger.LogInformation("Sufficient ecash balance, using Swap (NUT-03) to create token from existing proofs");
                var token = await SwapProofsAsync((ulong)amountSatoshis, mintUrl, storeId);
                if (token != null)
                {
                    _logger.LogInformation("Token created successfully using Swap from existing ecash balance");
                    return token;
                }
                else
                {
            ...
            // 3b. If ecash balance is insufficient, mint from Lightning
            ...
            _logger.LogInformation("Minting {Amount} sat from Lightning (NUT-04) to supplement ecash balance", neededAmount);
            ...
            _logger.LogInformation("Combining existing proofs ({EcashBalance} sat) with newly minted proofs ({NewAmount} sat)", 
                ecashBalance, neededAmount);
            
            var allProofs = new List<object>();
            
            // Get existing proofs if we have any
            if (ecashBalance > 0)
            {
            ...
            if (finalToken != null)
            {
                _logger.LogInformation("Token created successfully from combined proofs ({TotalProofs} proofs, {TotalAmount} sat)", 
                    allProofs.Count, amountSatoshis);
            }
            else
            {
