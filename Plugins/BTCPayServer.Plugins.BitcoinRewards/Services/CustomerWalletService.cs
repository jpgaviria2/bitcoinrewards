...
public class CustomerWalletService
{
        ...
        Guid fromWalletId, string toUsername, long amountSats, string? memo, string storeId)
    {
        ...
        // Use execution strategy to wrap the transaction (required by NpgsqlRetryingExecutionStrategy)
        await using var ctx = _dbFactory.CreateContext();
        var strategy = ctx.Database.CreateExecutionStrategy();

        return await strategy.ExecuteAsync<(bool Success, string? Error, long? FromSatsBalance, long? FromCadBalanceCents)>(async () =>
        {
